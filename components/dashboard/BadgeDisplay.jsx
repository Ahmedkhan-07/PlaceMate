...
const BADGE_DEFS = {
    'first-login': { label: 'First login', emoji: '🎉', color: 'text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 border-blue-200' },
    ...
    'mock-champion': { label: 'Mock champion', emoji: '🏆', color: 'text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border-amber-200' },
    'flashcard-pro': { label: 'Flashcard pro', emoji: '📚', color: 'text-sky-700 bg-sky-50 border-sky-200' },
    'top-10': { label: 'Top 10', emoji: '🌟', color: 'text-yellow-700 dark:text-yellow-300 bg-yellow-50 dark:bg-yellow-900/30 border-yellow-200' },
};
    ...
    return (
        <div className="bg-white dark:bg-gray-800 border border-border dark:border-gray-700 rounded-xl p-5 shadow-card">
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs text-text-secondary dark:text-gray-400">{badges.length} earned</span>
            </div>
            {badges.length === 0 ? (
                <div className="text-center py-4">
                    <Award className="h-8 w-8 text-slate-300 mx-auto mb-2" />
                    <p className="text-xs text-text-secondary dark:text-gray-400">Complete challenges to earn badges</p>
                </div>
            ) : (
                <div className="flex flex-wrap gap-2">
                    {badges.map((badge, i) => {
                        const def = BADGE_DEFS[badge] || { label: badge, emoji: '🏅', color: 'text-slate-700 dark:text-gray-300 bg-slate-50 border-slate-200' };
                        return (
                            <div
                                key={i}
